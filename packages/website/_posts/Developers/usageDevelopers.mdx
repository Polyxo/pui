...
```bash
npm install @progressiveui/react @progressiveui/styles --save
```

To install the Kit through `yarn`, run:

```bash
yarn add @progressiveui/react @progressiveui/styles
```
...
```bash
npm install @progressiveui/themes-core --save-dev
```
...
Wrap your Application in the [WFPCoreProvider](/documentation/developing/themes) to apply global settings.
...
```bash
npm install @progressiveui/react@alpha  @progressiveui/styles@alpha
```
