...
- [Checkboxes](/components/controls/checkbox) When multiple selections are allowed.
- [Select](/components/forms/select) For conserving space when dealing with a large set of options.
- [Toggle Switches](/components/controls/toggle) When a binary choice (on/off) is more appropriate than a list of options.
