...
`<PasswordInput />` extends the Text Input pattern with an inline toggle so users can reveal or hide sensitive values without retyping. Import it from `@progressiveui/react` and provide the same props you would for `TextInput`. The component handles switching between `password` and `text` input types, preserves helper text and validation messaging, and exposes two accessibility labels—`showPasswordLabelText` and `hidePasswordLabelText`—to describe the toggle button’s state.

```tsx
import { PasswordInput } from "@progressiveui/react";
